...
class App {
    ...
    setupEventListeners() {
        ...
        // Batch send to Firefly
        const sendAllFireflyBtn = document.getElementById('send-all-firefly-btn');
        if (sendAllFireflyBtn) {
            sendAllFireflyBtn.addEventListener('click', () => {
    ...
    async sendAllToFirefly() {
        ...
        const btn = document.getElementById('send-all-firefly-btn');
        if (!btn) return;
        ...
        try {
            ...
            if (data.success) {
                ...
                // Save to history using centralized service
                if (window.HistoryService) {
                    await window.HistoryService.saveBatch('firefly', allPrompts);
                }
                
                if (failed === 0) {
                    window.Utils.showToast(`✅ All ${successful} prompts sent to Firefly!`, 'success');
                } else {
                    window.Utils.showToast(`⚠️ Sent ${successful}/${allPrompts.length} prompts. ${failed} failed.`, 'warning');
                }
